...
const HoldPieceDisplay = ({ piece, canHold, blockSize = 20 }) => {
  const canvasRef = useRef(null);
  
  // キャンバスのサイズ
  const width = 6 * blockSize;
  ...
  useEffect(() => {
    ...
    // ホールドができないときは半透明のオーバーレイを描画
    if (!canHold && piece) {
      drawDisabledOverlay(ctx);
    }
  }, [piece, canHold, blockSize, width, height]);
  ...
  return (
    <div className={styles.pieceDisplay}>
      <div className={styles.title}>HOLD</div>
